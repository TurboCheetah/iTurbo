  ...
  async run(ctx, args) {
    if (!ctx.guild || args[0] === 'user') {
      const embed = new MessageEmbed()
        ...
        .addField(ctx.author.settings.prefix.length > 1 ? 'Prefixes' : 'Prefix', ctx.author.settings.prefix.length > 1 ? ctx.author.settings.prefix.map(p => `\`${p}\``).join(', ') : 'None set', true)
