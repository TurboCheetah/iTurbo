...
class Poll extends Command {
  ...
  async run(ctx, [channeltxt, ...choices]) {
    ...
    if(!args.length) return ctx.reply(`You need to provide me arguments. Usage: \`${ctx.guild.settings.prefix}${this.usage}\``);

    // if(!channel.postable) throw "I cannot send message in that channel.";
    if(!channel.permissionsFor(ctx.member).has("SEND_MESSAGES")) return ctx.reply("You can't post in that channel.");
