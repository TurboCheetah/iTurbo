...
class Load extends Command {
  ...
  async run (ctx, [store]) {
    ...
    try {
      ...
      return ctx.reply(`${this.client.constants.success} Successfully reloaded/loaded ${store.size} ${store.name}. ${after === 0 ? 'There was nothing new right?' : `${after} new ${store.name} were loaded.`}`)
    } catch (err) {
