...
class Store extends Command {
  ...
  async sell(ctx, args) {
    ...
    return ctx.reply(`${this.client.constants.emojis.success} Successfully sold the role **${role.name}** for **¥${refund.toLocaleString()}** refund.`)
  }
  ...
  async buy(ctx, args) {
    ...
    return ctx.reply(`${this.client.constants.emojis.success} Successfully bought the role **${role.name}** for **¥${price.toLocaleString()}**`)
  }
  ...
  async delete(ctx, args) {
    ...
    return ctx.reply(`${this.client.constants.emojis.success} Successfully removed the role **${role.name}** from the store.`)
  }
  ...
  async add(ctx, [price, ...args]) {
    ...
    if (price >= Number.MAX_SAFE_INTEGER) return ctx.reply('That price is too high.')
    ...
    return ctx.reply(`${this.client.constants.emojis.success} Success! **${role.name}** is now on sale for **¥${price.toLocaleString()}**`)
  }
