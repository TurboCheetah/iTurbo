...
class Pokemon extends Command {
  ...
  async run (ctx) {
    ...
    if (!attempts || !attempts.size) {
      await msg.delete()
      return ctx.reply(`Ba-You took too long to answer. It was ${pokemon.name}.`)
    }
    ...
    await msg.delete()
    return ctx.reply(`Ba-You answered incorrectly, It was **${this.client.utils.toProperCase(pokemon.name)}.**`)
  }
}
