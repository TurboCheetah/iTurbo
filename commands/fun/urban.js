...
class Urban extends Command {
  ...
  async run(ctx, args) {
    ...
    if (isNaN(parseInt(ind))) return ctx.reply('Page must be a number.')
    const index = ind - 1
    ...
    if (typeof result === 'undefined') {
      return ctx.reply(index === 0 ? 'I could not find this entry in Urban Dictionary' : 'I could not find this page in Urban Dictionary, try a lower page index')
    }
    ...
    return ctx.reply(
      new MessageEmbed()
        ...
        .setDescription([`→ \`Definition\` :: ${ind}/${list.length}\n${definition}`, `→ \`Example\` :: ${this.example(result.example)}`, `→ \`Author\` :: ${result.author}`])
