...
class Enchant extends Command {
  constructor(...args) {
    super(...args, {
      description: language => language.get('enchantDescription'),
      usage: language => language.get('enchantUsage'),
      aliases: ['enchantify']
    })
  }

  run(ctx, args) {
    if (!args.length) return ctx.reply(ctx.language.get('enchantNoArgs'))
