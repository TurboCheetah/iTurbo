...
class Warn extends Command {
  ...
  async run(ctx, [user, ...reason]) {
    ...
    if(user.id === ctx.author.id) return ctx.reply("You can't warn yourself.");
    if(user.id === this.client.user.id) return ctx.reply("Why would you try to warn me?");
    if(user.bot) return ctx.reply("You can't warn bots.");
    if(user.id === ctx.guild.ownerID) return ctx.reply("You can't warn the owner.");
