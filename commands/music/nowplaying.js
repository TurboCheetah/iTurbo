...
class NowPlaying extends Command {
  ...
  async run (ctx) {
    ...
    const embed = new MessageEmbed()
      ...
      .addField('Duration', `\`${queue.formattedCurrentTime}\` [${createBar(queue.songs[0].duration, Math.floor(queue.currentTime / 1000), 20)[0]}] \`${queue.songs[0].formattedDuration}\``, false)
    ctx.reply({ embed })
  }
}
