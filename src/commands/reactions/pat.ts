...
export abstract class PatCommand {
  ...
  async pat(
    ...
    if (member === (interaction.member as GuildMember)) return await interaction.reply({ content: "You can't pat yourself!", ephemeral: true })

    await interaction.deferReply()

    const { url } = await client.taihou.toph.getRandomImage('pat', { nsfw: isNSFW(interaction.channel as TextBasedChannels) })
