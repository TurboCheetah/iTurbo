...
export abstract class TickleCommand {
  ...
  async tickle(
    ...
    if (member === (interaction.member as GuildMember)) return await interaction.reply({ content: "You can't tickle yourself!", ephemeral: true })

    await interaction.deferReply()

    const { url } = await client.taihou.toph.getRandomImage('tickle', { nsfw: isNSFW(interaction.channel as TextBasedChannels) })
