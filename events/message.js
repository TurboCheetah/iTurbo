...
class MessageEvent extends Event {
  ...
  async run (msg) {
    ...
    if (msg.content === this.client.user.toString() || (msg.guild && msg.content === msg.guild.me.toString())) { return msg.channel.send(`Hi! Run \`${prefix}help\` to get a list of commands you can use.`) }
    ...
    const prefixMatch = new RegExp(`^(?:(?:(?:hey|yo|ok),? )?iturbo,? )|^<@!?${this.client.user.id}> |^${
    ...
    if (typeof rl === 'string') { return msg.channel.send(rl) }

    // Command checks.

    if (command.ownerOnly && msg.author.id !== this.client.constants.ownerID) { return msg.channel.send("What do you think you're doing? That command is only for my master!") }

    // Check for NSFW channel. NSFW is allowed in DMs
    if (command.nsfw && msg.guild && !msg.channel.nsfw) { return msg.channel.send(this.client.utils.random(this.client.responses.notNSFWChannel)) }

    if (command.guildOnly && !msg.guild) { return msg.channel.send('This command can only be used in a guild. What are you doing sliding in my DMs?') }

    if (!command.enabled && msg.author.id !== this.client.constants.ownerID) { return msg.channel.send('My master has ordered me to disable that command so I cannot let you use it!') }

    if (command.category === 'Social' && !msg.guild.settings.social) { return msg.channel.send('The social economy system has been disabled in this server by an Admin so I cannot let you use that command.') }
    ...
    if (command.cost && msg.guild && msg.guild.settings.social) {
      ...
      if (balance < cost) { return msg.channel.send(`You need **¥${cost}** to run that command but you only have **¥${balance}**.${claim}`) }
      // Deduct.
